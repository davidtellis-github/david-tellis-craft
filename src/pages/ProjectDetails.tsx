...
const ProjectDetails: React.FC = () => {
  ...
  if (!project) {
    return <div className="min-h-screen bg-background text-foreground flex items-end justify-center">
        <div className="text-end">
          ...
          <Link to="/portfolio" className="inline-flex items-center gap-2 bg-foreground text-background px-6 py-3 rounded-full hover:bg-foreground/90 transition-colors">
  ...
  return <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="fixed top-6 right-6 z-50">
        <button onClick={() => navigate(-1)} className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-muted/80 backdrop-blur-sm hover:bg-muted transition-colors">
      ...
      <main className="w-[98vw] mx-auto px-4 md:px-6">
        ...
        <section ref={heroAnim.ref} id="overview" className="">
            ...
            {project.videoUrl ? <div className="relative aspect-video rounded-lg overflow-hidden mb-20">
                <iframe src={`${project.videoUrl}&background=1`} className="absolute inset-0 w-full h-full" frameBorder="0" allow="autoplay" title={`${project.title} Video`} />
              </div> : project.mockupImages && project.mockupImages.length > 0 ? <div className="relative rounded-2xl overflow-hidden shadow-2xl shadow-background/20 mb-20">
                <img src={mockupImageMap[project.mockupImages[0]] || project.mockupImages[0]} alt={`${project.title} interface mockup`} className="w-full h-auto object-cover" />
              </div> : <div className="relative aspect-video bg-muted/50 rounded-2xl overflow-hidden group cursor-pointer mb-20">
        ...
        <section ref={roleAnim.ref} id="role" className={`min-h-[80vh] flex flex-col justify-start pt-[20vh] transition-all duration-1000 ${roleAnim.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="grid md:grid-cols-2 gap-12">
            <div>
              ...
              {project.id === "wedding-verse" ? <ul className="space-y-3">
            ...
            <div className="space-y-6">
              ...
              {project.id === "wedding-verse" && <div className="pt-4 border-t border-border/20">
        ...
        <section ref={challengeAnim.ref} id="challenge" className="">
          ...
          <div className="grid md:grid-cols-2 gap-16">
            ...
            <div className="space-y-6">
              ...
              {project.id === "wedding-verse" && <div className="bg-primary/5 border border-primary/10 rounded-xl p-6">
          ...
          {/* Solution Image */}
          {project.mockupImages && project.mockupImages[1] && <div className="rounded-2xl overflow-hidden shadow-xl shadow-background/20 mt-16">
              <img src={mockupImageMap[project.mockupImages[1]] || project.mockupImages[1]} alt={`${project.title} solution mockup`} className="w-full h-auto object-cover" />
            </div>}
        </section>

        {/* Core Features Delivered */}
        <section ref={featuresAnim.ref} id="features" className={`min-h-screen flex flex-col justify-start py-[20vh] transition-all duration-1000 ${featuresAnim.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="mb-20">
        ...
        <section ref={howIWorkedAnim.ref} id="how-i-worked" className={`min-h-screen flex flex-col justify-start py-[20vh] transition-all duration-1000 ${howIWorkedAnim.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="mb-24">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {project.process.map((phase, index) => <div key={index} className="relative">
        ...
        {(project.iterations || project.videoUrl) && <section ref={designEvolutionAnim.ref} id="design-evolution" className={`min-h-screen flex flex-col justify-start py-[20vh] transition-all duration-1000 ${designEvolutionAnim.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <h2 className="text-3xl md:text-5xl font-light mb-20">Design Evolution</h2>
            
            {/* Iterations */}
            {project.iterations && <div className="mb-24">
                <div className="mb-12">
                  <h3 className="text-2xl md:text-3xl font-light mb-4">Iterations & UX Decisions</h3>
                  {project.iterations.intro && <p className="text-base md:text-lg text-muted-foreground leading-relaxed font-light">
                      {project.iterations.intro}
                    </p>}
                </div>

                {project.iterations.note && <div className="bg-muted/30 rounded-lg p-4 mb-8">
                ...
                <div className="columns-1 sm:columns-2 lg:columns-3 gap-4 space-y-4">
                  {project.iterations.images.map((imagePath, index) => <div key={index} className="break-inside-avoid cursor-pointer group">
                      <div className="rounded-xl overflow-hidden bg-muted/20">
                        <img src={mockupImageMap[imagePath] || imagePath} alt={`Design Iterations ${index + 1}`} className="w-full h-auto object-cover transition-transform duration-300 group-hover:scale-105" />
                      </div>
                    </div>)}
                </div>
              </div>}
            ...
            {project.videoUrl && <div>
                ...
                <div className="relative aspect-video bg-muted/50 rounded-xl overflow-hidden shadow-2xl shadow-background/20">
                  <iframe src={project.videoUrl} className="absolute inset-0 w-full h-full" frameBorder="0" allow="autoplay; fullscreen; picture-in-picture; clipboard-write" title={`${project.title} Video Walkthrough`} />
                </div>
              </div>}
          </section>}

        {/* Impact - Outcomes + Learnings Combined */}
        <section ref={impactAnim.ref} id="impact" className={`min-h-screen flex flex-col justify-start py-[20vh] transition-all duration-1000 ${impactAnim.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="mb-24">
            ...
            {project.id === "wedding-verse" ? <div className="space-y-8">
          ...
          <div>
            ...
            <div className="max-w-4xl">
              {project.id === "wedding-verse" ? <div className="space-y-6">
                  <div className="flex items-start gap-4">
        ...
        <section ref={galleryAnim.ref} id="gallery" className={`min-h-screen flex flex-col justify-start py-[20vh] transition-all duration-1000 ${galleryAnim.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          {/* Gallery */}
          <div className="mb-24">
            <h2 className="text-3xl md:text-5xl font-light mb-20">UI Gallery</h2>
            {!isLoading && allUIAssets.length > 0 && <UIGallery assets={allUIAssets} projectTitle={project.title} />}
            {!isLoading && allUIAssets.length === 0 && <p className="text-muted-foreground text-center py-12">
          ...
          <div className="text-center border-t border-border/10 pt-24">
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              {project.links.live && <a href={project.links.live} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-foreground text-background px-8 py-4 rounded-full hover:bg-foreground/90 transition-colors text-base font-normal">
                  <ExternalLink className="h-5 w-5" />
                  Live Project
                </a>}
              {project.links.figma && <a href={project.links.figma} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 border border-border px-8 py-4 rounded-full hover:bg-muted/50 transition-colors text-base font-normal">
                  <Figma className="h-5 w-5" />
                  Figma
                </a>}
              {project.links.playstore && <a href={project.links.playstore} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 border border-border px-8 py-4 rounded-full hover:bg-muted/50 transition-colors text-base font-normal">
